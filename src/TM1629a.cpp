...
bool TM1629a::getButton(button_t s){
  _buttons = getButtons();
  return bitRead(_buttons, s);
}

// buttons K3/KS1-8
uint8_t TM1629a::getButtons(){
...
void TM1629a::displayVal(uint8_t digitId, uint8_t val){
  if (digitId>15| val>15 | val<0) return;
  setDisplayMode(DISPLAY_TURN_ON | _pulse);
  setDataInstruction(INSTRUCTION_WRITE_DATA| INSTRUCTION_ADDRESS_FIXED);
  writeDataAt(FIRST_DISPLAY_ADDRESS+14-digitId, _digits[val]);
}
